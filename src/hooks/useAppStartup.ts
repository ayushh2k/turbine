...
export function useAppStartup(createWorkspace: (name?: string) => unknown) {
  ...
  useEffect(() => {
    ...
    async function init() {
      ...
      if (workspaceState.workspaces.length === 0 && cliArgs.length > 1 && cliArgs[1]) {
        // First launch with CLI directory arg — create a workspace for it
        const initialDir = cliArgs[1];
        const ws = createWorkspace('Workspace 1') as unknown as { id: string };
        useWorkspaceStore.setState((s) => ({
          workspaces: s.workspaces.map((w) =>
            w.id === ws.id ? { ...w, panes: w.panes.map((p) => ({ ...p, workingDirectory: initialDir })) } : w
          ),
        }));
      } else if (workspaceState.workspaces.length > 0 && cliArgs.length > 1 && cliArgs[1] && !cliArgs[1].includes('tauri-dev')) {
